 ...
 */
async function fetchSeoInfo(parameters: SeoParameters) {
 ...
 */
async function analyzeGeoOptimization(parameters: GeoParameters) {
  const { url } = parameters;
  ...
  try {
    ...
    jsonLdScripts.each((_, element) => {
      try {
        ...
        if (jsonData['@type']) {
          ...
          if (types.includes('FAQPage')) geoIndicators.hasFAQSchema = true;
          if (types.includes('HowTo')) geoIndicators.hasHowToSchema = true;
          if (types.includes('QAPage')) geoIndicators.hasQAPageSchema = true;
        }
    ...
    Object.entries(scoreFactors).forEach(([key, value]) => {
      if (key === 'hasGoodWordCount') {
        geoScore += value;
      } else if (geoIndicators[key as keyof typeof geoIndicators]) {
        geoScore += value;
      }
    });
